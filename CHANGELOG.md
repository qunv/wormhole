...
- Added staged zero-downtime remote MCP bearer rotation with optional `authTokenFallbackEnv`, dual-token listener acceptance, fallback-only startup safety, credential-level readiness probes, fallback-first public verification, and an Admin rotation flow that promotes the staged credential only after the running listener accepts it.
