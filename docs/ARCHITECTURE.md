...
Configured `mcpServers` entries are materialized after the six built-in modules. `workspaceIds` filters entries before any transport work; an empty list or `*` applies to every runtime. Primary and named runtimes initialize concurrently, while one runtime's configured dependencies remain ordered. The CLI readiness budget therefore uses the slowest workspace dependency chain rather than summing every workspace.

`SharedServices` resolves a connection key from server name, transport settings, timeouts, referenced-secret fingerprints, and—only for stdio—the confined resolved `cwd`. Workspace scope, required state, startup mode, tool filtering, and policy do not change connection identity. Compatible runtimes reuse one long-lived `upstreammcp.Client` and MCP session. Concurrent acquisition is single-flight per connection key; failures are cached for `failureCooldownMs` so multiple workspace runtimes do not repeatedly start the same unavailable process. Tool filtering and approval policy form a separate contract key, so policy-only differences create different `upstreamMCPModule` contracts while sharing the same client.

`startupMode=eager` performs MCP initialization and paginated `tools/list` before readiness. `background` and `lazy` first load a persistent raw tool catalog, validate and bound it through the same module contract builder, namespace names as `<server>__<normalized_name>`, and register immutable `ToolSpec` values before the downstream MCP servers are constructed. `background` opens the transport asynchronously; `lazy` opens it on the first call. Required servers always resolve to eager mode. A cache miss or invalid catalog falls back to one eager bootstrap and atomically replaces the catalog.

Deferred clients run a live `tools/list` on their first connection and save the refreshed raw contract for the next daemon restart. The current downstream contract is never mutated in place because clients may cache `tools/list`. An authenticated Admin refresh creates a fresh upstream session, discovers and persists the live catalog, retires the prior session only after active borrowers release it, and reports active/cached/live contract hashes plus added, removed, and changed public tools. A changed live contract is marked restart-required rather than being registered dynamically. Catalogs persist only minimal names, titles, descriptions, input schemas, and annotations under `state/upstream-mcp/catalogs`; arbitrary metadata, credentials, calls, arguments, and results are excluded. Files are owner-only, individually bounded, atomically replaced, limited to 64 entries, and pruned after 90 days when a catalog is saved.
...
Remote-ingress runtime status crosses the `server -> admin` boundary through an injected callback rather than an `admin -> server` dependency. `GET /admin/api/v1/remote-ingresses` caps the result at 32 active ingresses and gives the provider an overall bounded context. The server checks only the configured loopback listener: secret presence is reduced to booleans, TCP reachability is tested locally, and a short authenticated MCP handshake plus `tools/list` reports protocol/tool-count readiness. The probe never requests `publicUrl`, never returns bearer/provider values, and intentionally does not claim that an external HTTPS publisher is healthy. The React Configuration editor combines this active snapshot with write-only secret presence, registered workspace/profile catalogs, and the unsaved draft so runtime-vs-draft restart mismatches remain visible.
...
The Admin diagnostic endpoint emits an attachment capped at 4 MiB. It strips runtime and approval tokens, replaces direct upstream environment/header values with key lists, applies structured and unstructured redaction, removes audit arguments/session identities, limits audit records and log tails, and replaces every currently referenced environment secret by exact value before encoding.
...
Remote MCP ingresses are a separate downstream boundary. Each enabled ingress is daemon-owned, binds only to `127.0.0.1:<localPort>`, mounts only `/mcp`, and requires at least one configured bearer credential. `mode: fixed` is the backward-compatible default: it resolves one workspace at startup and constructs one immutable selected tool-profile server. `mode: session` instead attaches `/mcp` to the daemon's existing `SessionRouter`, exposes the four workspace control tools across all currently registered runtimes, and is security-locked to the built-in `remote-read` profile; `workspaceId` is invalid in this mode. The remote listener still does not mount Admin, `/healthz`, `/internal/healthz`, the literal `/mcp/session` path, or any fixed-workspace compatibility endpoint. `remote-read` deliberately excludes execution and mutation tools. Session requests use the router's conservative minimum body limit across routed runtimes. All ingress sockets are bound before the main listener begins serving, so an invalid mode/workspace/profile, missing bearer, or occupied ingress port fails startup before the supervisor health endpoint becomes ready.

Dedicated ingresses use the official MCP Go SDK's stateless Streamable HTTP handler and negotiate MCP `2026-07-28` when the peer supports it while retaining older protocol compatibility. `POST /mcp` is the request/response path. Authenticated `GET /mcp` reaches the MCP transport so older clients may request the optional standalone SSE channel; the stateless handler may return `405` when no stream is available. When an `Origin` header is present, the ingress validates it against the configured `publicUrl` origin and returns `403` on mismatch. If `publicUrl` is absent, Origin-bearing requests fail closed; server-to-server traffic without Origin is unaffected.

The publisher is independent from the MCP listener. `provider: external` is the generic default and creates no child process, allowing Caddy, Nginx, Tailscale, Cloudflare, or another operator-owned HTTPS layer to publish only the dedicated port. The managed `cloudflare` provider launches `cloudflared tunnel --no-autoupdate run` and supplies the remotely managed tunnel credential through `TUNNEL_TOKEN` in the child environment rather than argv. Public-hostname routing remains provider-owned and must target the dedicated loopback port, never the main Admin/MCP listener. Provider and MCP bearer credentials use different environment references and participate in daemon ConfigID only through one-way fingerprints. `doctor` performs an authenticated local MCP handshake plus `tools/list` after socket/auth checks, so readiness verifies the protocol contract rather than TCP reachability alone.

The Admin UI may generate a remote MCP bearer only as an explicit browser action after its environment reference is already persisted. The browser uses Web Crypto, writes the generated value through the same write-only secret mutation endpoint used by manual secret entry, and retains the cleartext only in ephemeral component state for one-time copy. The server still has no secret-read API, diagnostic/config responses remain value-free, and changing a stored bearer affects the active remote listener only after normal daemon reconciliation/restart. The connection kit reports only local listener/contract readiness; it does not make implicit outbound requests to the configured public URL, avoiding an Admin-driven SSRF surface.

Remote bearer rotation has a two-slot compatibility model. `authTokenEnv` remains the normal primary credential and is the only field required by existing configurations. An optional `authTokenFallbackEnv` is used only during rotation. When both referenced values are present at daemon startup, the listener accepts either Bearer value using constant-time comparisons; if the primary value is removed during finalization, a valid fallback alone is sufficient for startup. Secret fingerprints for both slots participate in ConfigID without embedding either value. Admin secret references include both slots, so the staged token remains write-only and can only be created after its reference is persisted.

Rotation readiness is credential-aware rather than presence-only. The local Admin status provider independently authenticates with the current primary and fallback environment values and reports whether each one is actually accepted by the running listener. This catches the case where a staged secret is regenerated after the last restart: the environment contains a value but the listener still holds the previous value. The UI permits promotion only after the configured primary and staged fallback both authenticate against the current listener. Promotion deletes the old value only when Wormhole owns it in `.env`, then changes the draft so the staged reference becomes primary; a final restart removes the old credential from the listener. If an interruption occurs after deleting a managed primary but before persisting promotion, the persisted fallback reference/value still allows the daemon to start.

Public-route verification is a separate explicit CLI boundary. `wormhole remote verify <name>` loads only a configured ingress, authenticates first to its loopback `/mcp` listener and then to its validated `publicUrl`, disables HTTP redirects, bounds tool pagination, and compares the negotiated protocol plus a deterministic SHA-256-derived hash of the complete discovered tool metadata. This is identical for fixed and session ingresses because the dedicated public surface remains `/mcp` in both modes. During a staged rotation it prefers the fallback credential so the operator validates the new bearer before changing the hosted client. The command fails when either endpoint is unavailable, authentication fails, the public URL is absent, or the public contract differs. It is never run by daemon startup, health checks, or Admin polling, so arbitrary outbound verification cannot be triggered remotely through the local control plane.
...
beginToolCall (correlation ID + in-flight counters)
  → wait for the workspace-local bounded concurrency slot
