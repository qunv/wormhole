...
Remote MCP ingresses are a separate downstream boundary. Each enabled ingress is daemon-owned, binds only to `127.0.0.1:<localPort>`, mounts only `/mcp`, requires at least one configured bearer credential, resolves one workspace at startup, and constructs one immutable fixed tool-profile server. It does not mount Admin, `/healthz`, `/internal/healthz`, session routing, workspace selection, or another workspace endpoint. The built-in `remote-read` profile is the default and deliberately excludes execution and mutation tools. All ingress sockets are bound before the main listener begins serving, so an invalid workspace/profile, missing bearer, or occupied ingress port fails startup before the supervisor health endpoint becomes ready.
