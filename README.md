...
Bearer rotation is staged so the hosted client does not have to change credentials at the exact same moment as the daemon. **Start safe rotation** adds an optional `authTokenFallbackEnv`; after saving that reference, generate and copy the staged bearer, then **Save & Restart**. The remote listener now accepts both the existing primary bearer and the staged fallback. `wormhole remote verify <name>` prefers the staged bearer while rotation exists, so it can prove the new credential works through both the loopback and public HTTPS routes before you change Notion. After the hosted client is using the staged bearer, **Promote staged bearer** moves its environment reference to `authTokenEnv`; the final Save & Restart retires the old credential. A fallback-only credential is sufficient for startup during this transition, so deleting a Wormhole-managed old bearer cannot make a half-completed promotion unbootable.
