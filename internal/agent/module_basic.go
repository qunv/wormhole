...
func basicToolSpecs() []ToolSpec {
	empty := object(nil)
	return []ToolSpec{
		roSpec("ping", "Ping", "Check whether Wormhole is reachable.", object(map[string]any{"message": str("Optional echo message.")})),
		roSpec("workspace_info", "Workspace info", "Return roots, mode, policy, limits, and safety rules.", empty),
		roSpec("lca", "Wormhole status", "Short alias for workspace_info.", empty),
		...
		{
			Name: "cb_input", Title: "Wormhole input", Description: "Render the compact MCP Apps input widget.",
			...
			Meta: map[string]any{
				...
				"openai/toolInvocation/invoking": "Opening Wormhole input…",
				"openai/toolInvocation/invoked":  "Wormhole input ready.",
			},
		},
		...
		roSpec("profile_status", "Profile status", "Return the loaded .wormhole/profile.json.", empty),
		rwSpec("reload_profile", "Reload profile", "Reload .wormhole/profile.json from disk.", empty, false),
	}
}
