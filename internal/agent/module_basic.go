...
func basicToolSpecs() []ToolSpec {
	...
	return []ToolSpec{
		...
		roSpec("list_skills", "List skills", "List built-in and workspace skills.", empty),
		roSpec("read_skill", "Read skill", "Read one skill by name.", object(map[string]any{"name": str("Skill name.")}, "name")),
		rwSpec("create_skill", "Create skill", "Create or replace a workspace-local skill.", object(map[string]any{"name": str("Skill name."), "description": str("Short description."), "body": str("Markdown instructions.")}, "name", "body"), false),
		rwSpec("delete_skill", "Delete skill", "Delete a workspace-local skill.", object(map[string]any{"name": str("Skill name.")}, "name"), true),
		roSpec("workspace_search", "Workspace @ search", "Autocomplete files, folders, symbols, and skills.", object(map[string]any{"query": str("Picker query."), "path": str("Search root."), "include": array(str("")), "limit": integer()})),
