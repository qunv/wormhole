...
func TestToolRegistryContract(t *testing.T) {
	tools := Tools()
	if got, want := len(tools), 78; got != want {
	...
	for _, tool := range tools {
		...
		groups := 0
		for _, group := range []map[string]bool{basicTools, fsTools, execTools, figmaTools, repoTools, workflowTools} {
			if group[tool.Name] {
				groups++
			}
		}
	...
	for _, required := range []string{
		...
		"request_approval_batch", "figma_get_design_context", "lca_input",
	} {
		if !seen[required] {
