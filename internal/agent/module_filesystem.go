...
func filesystemToolSpecs() []ToolSpec {
	return []ToolSpec{
		...
		structuredROSpec("search_text", "Search text", "Search workspace text with ripgrep or a Go fallback.", object(map[string]any{"query": str("Text or regex."), "path": str("Search root."), "regex": boolean(), "glob": str("File glob."), "context": integer(), "limit": integer()}, "query")),
		...
		structuredROSpec("read_many", "Read many", "Read up to 100 files or line ranges in one call.", object(map[string]any{"paths": array(str("")), "requests": array(readRequestSchema()), "max_chars_per_file": integer(), "concurrency": integer()})),
		...
		rwSpec("apply_patch", "Apply patch", "Apply a unified diff or structured operations.", object(map[string]any{"diff": str("Unified diff."), "operations": array(patchOperationSchema())}), true),
		...
		roSpec("preview_patch", "Preview patch", "Dry-run a diff or structured operations.", object(map[string]any{"diff": str("Unified diff."), "operations": array(patchOperationSchema())})),
		roSpec("validate_patch", "Validate patch", "Return patch conflicts without writing.", object(map[string]any{"diff": str("Unified diff."), "operations": array(patchOperationSchema())})),
